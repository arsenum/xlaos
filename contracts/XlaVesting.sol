...
contract XlaVesting  {
    ...
    function unlock(address payable destAddr,uint256 amount) public payable{
       //require(msg.sender == owner, "Only owner can withdraw funds");
        //uint256 erc20balance = token.balanceOf(address(this));
        //require(amount <= erc20balance, "balance is low");
        ...
        //destAddr.transfer(amount);
        XOLA token = XOLA(this.XOLA_addr());
        token.allowance(msg.sender,address(this));
    ...
    // }
 function initContract(address _xla,address _xola) public {
    XLA_addr = _xla;
    XLA xla = XLA(_xla);
    XOLA_addr = _xola;
    XOLA xola = XOLA(_xola);
    //xla.increaseAllowance(address(this), amount);
    xla.allowance(msg.sender, address(this));
    xla.allowance( address(this),msg.sender);
    xola.allowance(msg.sender, address(this));
    xola.allowance( address(this),msg.sender);
}
    //ERC20 token,
    // function buy(uint256 amount, address order, address smartContract) public payable{
    //     //check
    //     //set order as paid
